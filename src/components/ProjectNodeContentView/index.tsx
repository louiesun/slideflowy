...
import { InnerProseMirrorEditorView, } from './InnerProseMirrorEditorView';
import { isTextSelection, posAtFirstLine, posAtLastLine, } from '../../services/ProseMirrorService';
import './style.scss';
...
export class ProjectNodeContentView extends React.Component {
    ...
    sendIndexFromSlide = (event) => {
        if (event.data.type !== 'SEND_INDEX_FROM_SLIDE')
            return;
    ...
    render() {
        return (React.createElement("div", { onClick: () => {
            ...
            }, ref: this.containerRef, className: classnames({
    ...
    renderEditor() {
        if (!this.props.editable ||
            ...
            return (React.createElement(InnerPreviewView, { className: classnames({
                    ProjectNodeContentView__content: true,
                    IsPlaying: this.props.node.id === this.props.currentIndex &&
                        (this.props.isSlideOpened || this.props.fromMindMap)
                }), dangerousHTML: this.props.node.content }));
        }
        return (React.createElement(InnerProseMirrorEditorView, { className: "ProjectNodeContentView__content", editorViewRef: this.editorViewRef, focusedAt: this.props.focusedAt, editorState: this.props.editorState, onStartEdit: this.onStartEdit, onEndEdit: this.onEndEdit, onDispatchTransaction: this.props.onDispatchTransaction, onApplyMark: this.props.onApplyMark, onCleanMark: this.props.onCleanMark, onReplaceTextWithMark: this.props.onReplaceTextWithMark, onRemoveStyle: this.props.onRemoveStyle, onKeyDown: this.onKeyDown, onPasteNodes: this.props.onPasteNodes, storeAnchor: this.storeAnchor }));
    }
    storeAnchor = (anchor) => {
    ...
    onKeyDown = (event) => {
        ...
        const isTopLevelNode = Boolean(isRootChildNode || (parentNode && parentNode.id === selectedRootNodeId));
        const isSelectedRootNode = this.props.node.id === this.props.selectedRootNodeId;
        const contextInfo = {
            isSelectedRootNode,
            isTopLevelNode,
        ...
        if ((osName() === 'macOS' && event.metaKey && event.shiftKey) ||
        ...
        if (event.key === 'ArrowUp' &&
            posAtFirstLine(this.editorViewRef.current, this.props.editorState && this.props.editorState.selection.to)) {
            this.props.onFocusSeeminglyPrevNode();
            event.preventDefault();
            return;
        }
        if (event.key === 'ArrowDown' &&
            posAtLastLine(this.editorViewRef.current, this.props.editorState && this.props.editorState.selection.to)) {
        ...
        // 其他平台 delete
        if ((osName() === 'macOS' && event.ctrlKey && event.key === 'd') ||
            event.key === 'Delete') {
            ...
            const isConcatAvailable = this.props.node.childrenIds.length === 0 &&
                this.props.nextNode &&
    ...
    onBackspaceDown(event, info) {
        ...
        if (!isTextSelection(selection) || !selection.$cursor)
            return;
        const prevNodeHasNoChild = this.props.prevNode && !this.props.prevNode.childrenIds.length;
    ...
    async keepScreenScrollProgress(works) {
        if (!this.containerRef.current || document.documentElement === null)
            return;
