...
export class InnerProseMirrorEditorView extends React.PureComponent {
    ...
    debouncedShowMenuBar = debounce(this.showMenuBar, SELECTION_CHANGE_TIMEOUT);
    hideMenuBar = () => {
        this.setState(produce((s) => {
    ...
    onEditorPaste = (editorView, event, slice) => {
        if (getClipboardImageFile(event))
            return true;
        const nodes = parseProjectNodeFromClipboardEvent(event, slice);
        if (nodes) {
    ...
    onEditorClick = (editorView, pos, event) => {
        if (event.target instanceof HTMLAnchorElement && event.target.href) {
             ...
             * 很不好看
             */
            if (/^https?:/.test(event.target.href)) {
    ...
    onKeyDown = (editorView, event) => {
        ...
        if (this.menuRef.current) {
            this.menuRef.current.onEditorKeyDown(event);
        }
    ...
    render() {
        return (React.createElement(React.Fragment, null,
            ...
            React.createElement(ProseMirrorMenuBar, { key: this.state.menuKey, ref: this.menuRef, editorState: this.props.editorState, style: this.state.menuStyle, onEditorRetrieveFocus: this.focusEditor, onApplyMark: this.props.onApplyMark, onCleanMark: this.props.onCleanMark, onRemoveStyle: this.props.onRemoveStyle, onBlur: this.onMenuBlur }),
