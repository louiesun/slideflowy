...
} from './ProseMirrorMenuBar'
import {
  ProseMirrorEditorView,
  ProseMirrorEditorViewProps,
} from '../ProseMirrorEditorView'
...
import {
  ...
  parseProjectNodeFromClipboardEvent,
} from '../../services/ProjectNodeService'
import debounce from 'lodash/debounce'
import { EditorView } from 'prosemirror-view'
import { Ref, createForwardingRef, assignRef } from '../../utils/R'
...
import {
  autoLinkAttrMark,
  getLinkInfoBySelection,
} from '../../services/ProseMirrorService/AutoLinkInputRule'
...
export interface InnerProseMirrorEditorViewProps {
  className?: string
  editorViewRef?: Ref<EditorView>
  editorState: EditorState
  focusedAt?: number
  onDispatchTransaction: ProseMirrorEditorViewProps['onDispatchTransaction']
  onKeyDown: (event: KeyboardEvent) => void
  onPasteNodes: (nodes: IProjectNode[]) => void
  onStartEdit: () => void
  onEndEdit: () => void
  onApplyMark: ProseMirrorMenuBarProps['onApplyMark']
  onCleanMark: ProseMirrorMenuBarProps['onCleanMark']
  onReplaceTextWithMark: (
    text: string,
    mark: MarkType,
    attrs: SimpleObj,
  ) => void
  onRemoveStyle: ProseMirrorMenuBarProps['onRemoveStyle']
  storeAnchor: (anchor: number) => void
}

interface InnerProseMirrorEditorViewState {
  menuKey: string
  menuStyle: React.CSSProperties
  linkEditorEditable: boolean
  linkEditorStyle: React.CSSProperties
  linkEditorState: {
    text: string
    link: string
  }
  blurBecauseOfOutsideClick?: boolean
  tooltipPosition?: {
    x: number
    y: number
  }
...
export class InnerProseMirrorEditorView extends React.PureComponent<
  ...
  InnerProseMirrorEditorViewState
> {
  readonly state: InnerProseMirrorEditorViewState = {
    menuKey: '' + Date.now(),
    menuStyle: {
      display: 'none',
      top: '-9999px',
  ...
  private editorRef = React.createRef<ProseMirrorEditorView>()

  private menuRef = React.createRef<ProseMirrorMenuBar>()
  private linkEditorRef = React.createRef<HTMLDivElement>()

  private editorViewRef = createForwardingRef<EditorView>(instance => {
    assignRef(instance, this.props.editorViewRef)
  })
  private tooltipText = $t('NUTFLOWY_TOOLTIP_OPEN_LINK').replace(
    '%s',
    osName() === 'macOS' ? 'Command' : 'Ctrl',
  )
  ...
  componentDidUpdate(prevProps: InnerProseMirrorEditorViewProps) {
    ...
    if (!prevProps.editorState.selection.eq(this.props.editorState.selection)) {
      this.onEditorSelectionChange(
        prevProps.editorState.selection,
  ...
  private debouncedStoreAnchor = debounce((newSelection: Selection) => {
  ...
  private onEditorSelectionChange(
    ...
    if (this.editorView?.hasFocus()) this.debouncedStoreAnchor(newSelection)
  }
   ...
   */
  private restoreFocusClickSource: Element | null = null
  ...
  private onDocumentClick = (event: MouseEvent) => {
    if (!(event.target instanceof Element)) return
    if (this.restoreFocusClickSource !== event.target) return
    this.restoreFocusClickSource = null
  }

  private tryEndEditingOnDocumentSelectionChange = () => {
    ...
    if (this.editorRef.current.contains(focusNode)) return
    if (!this.menuRef.current) return
    if (this.menuRef.current.contains(focusNode)) return
    if (!this.linkEditorRef.current) return
    if (this.linkEditorRef.current.contains(focusNode)) return
  ...
  private showMenuBar = () => {
    ...
    const containerRect = getElementRevPosRect(
      this.editorRef.current.containerElem.parentElement,
    )
    ...
    this.setState(
      produce((s: InnerProseMirrorEditorViewState) => {
        s.menuStyle = {
          display: 'block',
          top: `${pos.top}px`,
          left: `${pos.left}px`,
  ...
  private debouncedShowPopover = debounce(selection => {
  ...
  private onMenuBlur = () => {
    ...
    if (editorView && document.activeElement === editorView.dom) return
    this.props.onEndEdit()
  }

  private selectionOnEditLink: {
    type: string
    anchor: number
    head: number
  } | null = null
  ...
  private onEditLinkDone = (text: string, href: string) => {
    ...
    if (this.selectionOnEditLink && this.editorView) {
      ...
      this.editorView.dispatch(
        editorState.tr.setSelection(
          Selection.fromJSON(editorState.doc, this.selectionOnEditLink),
        ),
      )
  ...
  private onEditorPaste: ProseMirrorEditorViewProps['editorProps']['handlePaste'] = (
    editorView,
    ...
    const processNodes = async () => {
      ...
      for (const node of _nodes) {
        ...
        for (const image of _images) {
          ...
          if (imageData) {
            images.push({
              type: 'parasiticMedium' as 'parasiticMedium',
              data: imageData,
            })  
          }
        }
  ...
  private onEditorClick: ProseMirrorEditorViewProps['editorProps']['handleClick'] = (
    editorView,
    pos,
    event,
  ) => {
    ...
    if (anchorElement && anchorElement.href) {
      if (!ctrlPressManager.isPressed) return false
      event.preventDefault()
  ...
  private onKeyDown: ProseMirrorEditorViewProps['editorProps']['handleKeyDown'] = (
    editorView,
    event,
  ) => {
    ...
    return (
      <>
        ...
        <ProseMirrorMenuBar
          ...
          onEditorRetrieveFocus={this.focusEditor}
        ...
        <ProseMirrorLinkEditor
          ref={this.linkEditorRef}
          editable={this.state.linkEditorEditable}
          style={this.state.linkEditorStyle}
          state={this.state.linkEditorState}
          onDone={this.onEditLinkDone}
          onDelete={this.onEditLinkDelete}
          onBlur={this.onEditLinkBlur}
        />

        {this.state.tooltipPosition ? (
          <LinkTooltip
            text={this.tooltipText}
            mouseX={this.state.tooltipPosition.x}
            mouseY={this.state.tooltipPosition.y}
          />
        ) : null}
      </>
    )
...
const LinkTooltip = (props: {
  ...
  return (
    <div
      className="ProjectNodeContentView__LinkTooptip"
      style={{
        top: props.mouseY,
        left: props.mouseX,
      }}
    >
      <div className="ProjectNodeContentView__LinkTooptip-inner">
        {props.text}
      </div>
    </div>
  )
...
```
 */
export const createRestoreFocusElement = <E extends JSX.Element>(el: E): E => {
  if (typeof el.type !== 'string') {
    throw new TypeError(
      'createRestoreFocusElement only support native DOM, for example `createRestoreFocusElement(<img />)`',
    )
  }
