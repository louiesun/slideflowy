...
export class AdvancedSlide extends React.PureComponent {
    ...
    imgRelativePathMap = new Map();
    canvasRef = React.createRef();
    backgroundRef = React.createRef();
    imgViewerRef = React.createRef();
    lastNodeRef = React.createRef();
    topicsRef = React.createRef();
    backgroundUrl = this.props.slideBG;
    backgroundIndex = this.backgroundUrl
    ...
    bgSuffix = !webp ? SUFFIX_JPEG : SUFFIX_WEBP;
    // 处理content中的字体颜色，以便在暗色背景中能看得清
    brightenColor = (content) => {
        return (content
            // 清理因复制黏贴带过来的白色背景
            .replace(/background-color: rgb\(255, 255, 255\)/gi, '')
            .replace(/rgb\(([\d]*), ([\d]*), ([\d]*)\)/gi, match => COLOR_SCHEMA[match] || match)
            .replace(/rgba\(([\d]*), ([\d]*), ([\d]*), ([\d]*[/.]*[\d]*)\)/gi, match => COLOR_SCHEMA[match] || match)
            .replace(/rgb\(([\d]*), ([\d]*), ([\d]*)\)/gi, (match, p1, p2, p3, offset, string) => {
    ...
    buildContent = (node, classes) => (React.createElement("div", { className: classNames(classes) },
        node.content && (React.createElement(Content, { className: "text-content", dangerousHTML: this.brightenColor(node.content), onClick: e => this.onContentClick(e, node.id) })),
        node.images && node.images.length > 0 && (React.createElement(ImageList, { className: "img-content", previewUrls: node.imagePreviewUrls || [], images: node.images, withText: Boolean(node.content), imgViewerRef: this.imgViewerRef, canvasRef: this.canvasRef, scale: this.state.scale, verticalFit: this.state.verticalFit, isViewingImg: this.state.isViewingImg, startViewImg: (urls, index) => {
                this.setState({
                    isViewingImg: true,
                    previewUrls: urls,
    ...
    buildLevel1 = (node, index) => {
        ...
        const odd = index % 2;
        const l1Height = length > 5 ? contentHeight / 3 : contentHeight / Math.ceil(length / 2);
        const top = Math.floor(index / 2) * l1Height +
            l1Height / 2 +
            blankHeight -
        ...
        const isCurrentLevel = this.state.stepName === 'level' && this.state.depth === 1;
        const active = this.state.currentNodes[1] && node.id === this.state.currentNodes[1].id;
        const textAndImage = Boolean(node.content && node.images && node.images.length > 0);
        return (React.createElement("div", { className: classNames('level1', { active }), key: node.id, style: {
                transform: main || home || menu
            ...
            React.createElement("div", { className: classNames('title', 'l1'), onClick: e => {
                    e.stopPropagation();
                    e.preventDefault();
                    this.gotoByNodeId(node.id);
                } }, index < 9 ? '0' + (index + 1) : index + 1),
            node.content && (React.createElement(Content, { className: "indicator-content", dangerousHTML: this.brightenColor(extractFirstLineFromHtmlText(node.content)), onClick: e => this.onContentClick(e, node.id) })),
            ...
            React.createElement("div", { className: "next-level" }, this.buildLevel2(node, [...currentNodes]))));
    };
    buildLevel2 = (parent, ancestorNodes) => {
        const elements = [];
        if (parent.childrenIds && parent.childrenIds.length > 0) {
            ...
            parent.childrenIds.forEach((id, index) => {
                ...
                const parentActive = this.state.currentNodes[1] &&
                    this.state.currentNodes[1].id === parent.id;
                const selfActive = this.state.currentNodes[2] && this.state.currentNodes[2].id === id;
                const activeIndex = parentActive && this.state.currentNodes[2]
                ...
                const isCurrentLevel = this.state.stepName === 'level' && this.state.depth === 2;
                const textAndImage = Boolean(node.content && node.images && node.images.length > 0);
                let activeTranslateY = '';
                if (this.state.currentNodes[3] && selfActive) {
                    const seq3l = node.childrenIds.indexOf(this.state.currentNodes[3].id) + 1;
                ...
                elements.push(React.createElement("div", { className: classNames('level2', parentActive && selfActive ? 'active' : 'inactive'), key: node.id, style: parentActive
                    ...
                    React.createElement("div", { className: classNames('title', 'l2', {
                    ...
                    React.createElement("div", { className: "next-level" }, this.buildLevel3(node, [...currentNodes], num, transform))));
            });
        }
        return elements;
    };
    buildLevel3 = (parent, ancestorNodes, number, transformP) => {
        const elements = [];
        if (parent.childrenIds && parent.childrenIds.length > 0) {
            parent.childrenIds.forEach((id, index) => {
                ...
                const isCurrentLevel = this.state.stepName === 'level' && this.state.depth === 3;
                const textAndImage = Boolean(node.content && node.images && node.images.length > 0);
                const currentNodes = [...ancestorNodes];
                currentNodes[3] = node;
                this.steps.push({
                ...
                elements.push(React.createElement("div", { className: classNames('level3', {
                        active: this.state.currentNodes[3] &&
                            this.state.currentNodes[3].id === node.id,
                        closer: this.state.l3Index &&
                            Math.abs(index - this.state.l3Index) === 1,
                        faraway: this.state.l3Index &&
                            Math.abs(index - this.state.l3Index) === 2,
                        disappear: this.state.l3Index && Math.abs(index - this.state.l3Index) > 2,
                    }), key: node.id, style: {
                        transform: `translate(-80px, ${(index + 1) * 100}px)`,
                        transitionDelay: `${index * 0.1}s`,
                    } },
                    React.createElement("div", { className: classNames('title', 'l3') },
                        React.createElement("span", null, num)),
                    React.createElement("div", { className: "dot" }),
                    React.createElement("div", { className: "decorate-line-v" }),
                    this.buildContent(node, [
                        'content',
                        'l3',
                        {
                    ...
                    React.createElement("div", { className: "next-level" }, this.buildLevel4(node, [...currentNodes], transform))));
            });
        }
        return elements;
    };
    buildLevel4 = (parent, ancestorNodes, transformP) => {
        const elements = [];
        if (parent.childrenIds && parent.childrenIds.length > 0) {
            parent.childrenIds.forEach((id, index) => {
                const num = index + 1 + '';
                ...
                const isCurrentLevel = this.state.stepName === 'level' && this.state.depth === 4;
                const textAndImage = Boolean(node.content && node.images && node.images.length > 0);
                const currentNodes = [...ancestorNodes];
                currentNodes[4] = node;
                this.steps.push({
                ...
                elements.push(React.createElement("div", { className: classNames('level4', {
                        active: this.state.currentNodes[4] &&
                            this.state.currentNodes[4].id === node.id,
                        leaving: stepIndex === this.state.stepIndex - 1,
                        entering: stepIndex === this.state.stepIndex + 1,
                    }), key: node.id, style: { transform: `translate(77.5px, 0)` } },
                    React.createElement("div", { className: classNames('title', 'l4') },
                        React.createElement("span", null, num)),
                    this.buildContent(node, [
                        'content',
                        'l4',
                    ...
                    React.createElement("div", { className: "next-level" }, this.buildLevelGT4(node, [...currentNodes], 5, transform))));
            });
        }
        return elements;
    };
    buildLevelGT4 = (parent, ancestorNodes, depth, transformP) => {
        const elements = [];
        if (parent.childrenIds && parent.childrenIds.length > 0) {
            parent.childrenIds.forEach((id, index) => {
                const node = this.props.nodes[id];
                const isCurrentLevel = this.state.stepName === 'level' && this.state.depth === depth;
                const textAndImage = Boolean(node.content && node.images && node.images.length > 0);
                const currentNodes = [...ancestorNodes];
                ...
                elements.push(React.createElement("div", { className: classNames('levelGT4', {
                        active: this.state.currentNodes[depth] &&
                            this.state.currentNodes[depth].id === node.id,
                        isCurrentLevel,
                        leaving,
                    }), key: node.id },
                    React.createElement("div", { className: classNames('dot', {
                            isCurrentLevel,
                            leavingToNextLevel,
                        }) }),
                    this.buildContent(node, [
                    ...
                    React.createElement("div", { className: "next-level" }, this.buildLevelGT4(node, [...currentNodes], depth + 1, transformP))));
            });
        }
        return elements;
    };
    ...
    doTransform = (index) => {
        ...
        switch (step.name) {
            ...
            case 'level':
                if (step.depth === 1) {
                    if (step.node)
                        l1Index = props.rootNodeIds.indexOf(step.node.id);
                    bgTransform = `translate(${bgTranslateX}%, -35%) scale(4)`;
                }
                else {
                    if (step.depth === 3 && step.index)
                        l3Index = step.index;
                    bgTransform = `translate(
              calc(${bgTranslateX}% - ${1600 - step.transform.translateX}px),
              calc(-35% ${step.transform.translateY > 0
                        ? '- ' + step.transform.translateY
                        : '+ ' + Math.abs(step.transform.translateY)}px)
            ) scale(4)`;
                }
                break;
            default:
    ...
    restart = () => {
        if (this.animating || this.state.stepIndex === 0)
            return;
        this.doTransform(0);
    };
    prev = () => {
    ...
    debounceHideController = debounce(this.hideController, 5000);
    adjustContent = () => {
    ...
    throttledWheelHandler = throttle((event) => {
        const topics = this.topicsRef.current;
        if (topics &&
            ...
            if (event.deltaY > 0) {
                const newTranslateY = translateY - scrollStep < range[0]
                    ? range[0]
                ...
                const newTranslateY = translateY + scrollStep > range[1]
                    ? range[1]
                    : translateY + scrollStep;
                topics.style.transform = `translateY(${newTranslateY}px)`;
                topics.setAttribute('translateY', `${newTranslateY}`);
    ...
    render() {
        ...
        const { state, props } = this;
        return (React.createElement("div", { className: classNames('canvas', 'advanced', state.loading ? 'loading' : '', `${state.stepName}${state.depth > 0 ? (state.depth > 4 ? 'GT4' : state.depth) : ''}`), style: {
                transform: `translate(-50%, -50%) scale(${state.scale})`,
            }, ref: this.canvasRef, onClick: this.onClickPanel, onMouseMove: this.showController },
            React.createElement("div", { className: "background", ref: this.backgroundRef, style: { transform: state.bgTransform } }),
            React.createElement("div", { className: "last-node", ref: this.lastNodeRef },
                React.createElement(Content, { className: "text-content", dangerousHTML: state.lastNode
                        ? this.brightenColor(state.lastNode.content)
                        : props.fileName }),
                React.createElement("div", { className: "decorate-line" })),
            React.createElement("div", { className: "panel", onWheel: this.onTopicsWheel },
                React.createElement("div", { className: "container filename" },
                    React.createElement("h1", null, props.fileName),
                    React.createElement("div", { className: "decorate-line" })),
                React.createElement("div", { className: "topics", ref: this.topicsRef }, this.buildPanel())),
            React.createElement(Controller, { steps: this.steps, switchBG: this.switchBG, restart: this.restart, prev: this.prev, next: this.next, doTransform: this.doTransform, zoom: this.state.zoom, clickToZoom: this.clickToZoom, requestFullscreen: props.requestFullscreen, exitFullscreen: props.exitFullscreen, fullscreen: props.fullscreen, isPreview: props.isPreview, backgroundLoading: state.backgroundLoading, stepName: state.stepName, stepIndex: state.stepIndex, totalStep: this.steps.length, controller: state.controller, showController: this.showController, fileName: props.fileName, rootNodeIds: props.rootNodeIds, nodes: props.nodes }),
            React.createElement(ImageViewer, { imgRef: this.imgViewerRef, isViewingImg: this.state.isViewingImg, hideImgViewer: this.hideImgViewer, imgIndex: this.state.viewingImgIndex, previewUrlsLength: this.state.previewUrls.length, prevImg: this.prevImg, nextImg: this.nextImg }),
            React.createElement("div", { className: "end" }, $t('SLIDE_END'))));
    }
}
