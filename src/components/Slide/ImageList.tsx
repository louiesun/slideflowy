...
export interface ImageListProps {
  ...
  withText: boolean
  className?: string
  imgViewerRef: RefObject<HTMLImageElement>
  canvasRef: RefObject<HTMLDivElement>
  scale: number
  verticalFit: boolean
  isViewingImg: boolean
  startViewImg: (urls: string[], index: number) => void
}
...
export const ImageList: FC<ImageListProps> = (props) => {
  ...
  const imageResponse = (event: MessageEvent) => {
    if (
      event.data.type === 'IMAGE_RESPONSE' &&
  ...
  const fetchUrls = useCallback(
    async (images: UploadedImage[]) => {
      ...
      const mediumsPromise: Promise<any>[] = images.map(async (i) => {
    ...
    (event: React.MouseEvent<HTMLImageElement>) => {
      ...
      if (imgViewer && canvas) {
        ...
        imgViewer.onload = () => {
          ...
          props.startViewImg(
            previewUrls,
            previewUrls.findIndex((url) => img.src.indexOf(url) > -1),
          )
          ...
          setTimeout(() => {
            if (
              ...
              imgViewer.style.top = `${
                (canvas.clientHeight - img.naturalHeight) / 2
              }px`
              imgViewer.style.left = `${
                (canvas.clientWidth - img.naturalWidth) / 2
              }px`
            } else {
              if (
                canvas.clientWidth / img.naturalWidth >
                ...
                const width =
                  (img.naturalWidth * canvas.clientHeight) / img.naturalHeight
                ...
                const height =
                  (img.naturalHeight * canvas.clientWidth) / img.naturalWidth
  ...
  return (
    <div className={classNames(clsName, props.className)}>
      <div
        className={cls(
          '-container',
          { withText: props.withText },
          { multiImage: props.images.length > 1 },
          { moreThan2: props.images.length > 2 },
          { moreThan3: props.images.length > 3 },
          { moreThan4: props.images.length > 4 },
        )}
      >
        {previewUrls.map((url, index) => {
          ...
          return <img key={key} src={url} onClick={clickOnImage} />
        })}
      </div>
    </div>
  )
