...
export var ParasiticMedium;
(function (ParasiticMedium) {
    let utils;
    (function (utils) {
        function mimeTypeToExtName(types) {
            return types.map(t => t.split('/', 2)[1]).join(', ');
        }
        utils.mimeTypeToExtName = mimeTypeToExtName;
        function formatSize(sizes, unit = 'M') {
            let suffix = 'B';
        ...
        utils.formatSize = formatSize;
    })(utils = ParasiticMedium.utils || (ParasiticMedium.utils = {}));
    /**
    ...
    function pickAndUpload(err) {
        ...
        if (err instanceof PM.PickAndUpload.FailedError) {
            return transParasiticMediumUploadFailedError(err, $t('PARASITIC_MEDIUM_UPLOAD_READ_FILE_FAILED'));
        }
        return;
    }
    ParasiticMedium.pickAndUpload = pickAndUpload;
    /**
    ...
    ParasiticMedium.upload = upload;
})(ParasiticMedium || (ParasiticMedium = {}));
function transParasiticMediumUploadFailedError(err, fallback = $t('ERR_UNKNOWN')) {
    if (err.edge === KnownErrorEdge.nutstoreServer) {
        switch (err.errorCode) {
            ...
            default: {
                const suffix = err.extra && err.extra.status ? `(${err.extra.status})` : '';
                return $t('ERR_REQUEST_FAILED_WITH_ERROR_CODE').replace('{{errorCode}}', `server:${err.errorCode}${suffix}`);
            }
        }
    }
    if (err.edge === KnownErrorEdge.request) {
        if (err.errorCode === RequestErrorCode.NetworkTimeout) {
            ...
            return $t('ERR_REQUEST_FAILED_WITH_ERROR_CODE').replace('{{errorCode}}', `request:${err.errorCode}`);
        }
        else {
    ...
    if (err.errorCode) {
        return $t('ERR_UNKNOWN_WITH_ERROR_CODE').replace('{{errorCode}}', `${err.edge}:${err.errorCode}`);
    }
    else {
        return fallback;
    }
