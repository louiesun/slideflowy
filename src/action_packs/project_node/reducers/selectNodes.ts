...
export function selectNodes({ payload: { range, nodes: userSelectedNodes }, }, state) {
...
export function selectCopyNodes({ payload: { nodes: userSelectedNodes }, }, state) {
    if (!userSelectedNodes.length) {
        state.projectNodeUserSelectedNodeIds = [];
        state.projectNodeAllSelectedNodeIds = [];
        state.projectNodeAllSelectedCopyNodeIds = [];
 ...
 */
function flatAndSortNodes(userSelectedNodes, state) {
    // 过滤掉父节点已经在选区列表的节点，因为节点被选中后子节点一定属于被选中状态
    const isolateNodes = userSelectedNodes.filter(node => userSelectedNodes.every(_node => _node.childrenIds.indexOf(node.id) === -1));
    ...
    isolateNodes.sort((fstNode, sndNode) => {
        ...
        if (fstAncestorIds.indexOf(sndId) > -1)
            return 1;
        if (sndAncestorIds.indexOf(fstId) > -1)
            return -1;
        // 找到公共祖先，然后根据公共祖先的次级节点位置进行排序
        const fstAncestorIdAndIdxPairs = fstAncestorIds.map((id, idx) => [idx, id]);
        fstAncestorIdAndIdxPairs.reverse();
        const commAncestorInfo = reduce((ret, [fstAncestorIdx, id]) => {
            ...
            // [1,2,3,4]b
            const fstComparableAncestorId = fstAncestorIds[fstAncestorIdx + 1] || fstId;
            const sndComparableAncestorId = sndAncestorIds[sndAncestorIdx + 1] || sndId;
            return reduced({
                commonAncestorId: id,
                fstComparableAncestorId,
                sndComparableAncestorId,
            });
        }, null, fstAncestorIdAndIdxPairs);
        // 如果没有公共祖先，就对比根节点
        if (!commAncestorInfo) {
        ...
        return compareIds(commAncestorInfo.fstComparableAncestorId, commAncestorInfo.sndComparableAncestorId, commonParentChildrenIds);
    });
    return isolateNodes;
}
function compareIds(aId, bId, idOrder) {
...
function getAncestorIds(id, state) {
    if (id) {
