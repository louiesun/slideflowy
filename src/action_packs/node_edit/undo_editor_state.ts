...
export function undoEditorStateAvailable(id: IProjectNode['id'], state: State) {
  const status = projectNodeSelectors.getEditStatus(id, state)
  if (!status || !status.editorState) return false
  if (!undoDepth(status.editorState)) return false
  return true
}
 ...
 * * 如果没有历史可以撤销，返回 `false`
 */
export function undoEditorState(id: IProjectNode['id'], state: State) {
  if (!undoEditorStateAvailable(id, state)) return false
