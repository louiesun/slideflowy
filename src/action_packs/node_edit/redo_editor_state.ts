...
export function redoEditorStateAvailable(id: IProjectNode['id'], state: State) {
  const status = projectNodeSelectors.getEditStatus(id, state)
  if (!status || !status.editorState) return false
  if (!redoDepth(status.editorState)) return false
  return true
}
 ...
 * * 如果没有历史可以重做，返回 `false`
 */
export function redoEditorState(id: IProjectNode['id'], state: State) {
  if (!redoEditorStateAvailable(id, state)) return false
