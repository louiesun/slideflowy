...
export class FileService {
  async shareFile() {
    ...
    const { nutstoreFile } = store.getState() as State

    if (!nutstoreFile) return

    try {
      return await nutstoreFile.share()
    } catch (_err) {
      const message = (() => {
        if (_err instanceof errors.ApiError) return $t('GENERAL_ERROR_MSG')
  ...
  async getPreviewLink(mediumJson: ParasiticMedium.JSONObject) {
    const store = await createStore()
    const { nutstoreFile } = store.getState() as State
    if (!nutstoreFile) return
  ...
  async uploadImage(
    _file: File | string,
    onUploadProgress?: ParasiticMedium.Upload.Options['onUploadProgress'],
  ) {
    const store = await createStore()
    const { nutstoreFile } = store.getState() as State
    if (!nutstoreFile) return
    ...
    const limit = ParasiticMedium.fileExceededLimit(
    ...
    if (limit) {
      ...
      if (limit.type) {
        reason = $t('NUTFLOWY_IMAGE_UPLOAD_IMAGE_TYPE_ERROR').replace(
          '{{types}}',
          NutstoreSDKET.ParasiticMedium.utils.mimeTypeToExtName(limit.type),
        )
      } else if (limit.size) {
        reason = $t('NUTFLOWY_IMAGE_UPLOAD_IMAGE_SIZE_ERROR').replace(
          '{{size}}',
          NutstoreSDKET.ParasiticMedium.utils.formatSize(limit.size, 'M'),
        )
      }
      Notification.show({
        text: $t('NUTFLOWY_IMAGE_UPLOAD_FAILD$REASON').replace(
          '{{reason}}',
          reason,
        ),
      })
    ...
    try {
      ...
      const msg = $t('NUTFLOWY_IMAGE_UPLOAD_FAILD$REASON').replace(
        '{{reason}}',
        reason || $t('ERR_UNKNOWN'),
      )
        ...
        switch (data.type) {
            ...
            if (data.imageData) {
              ...
              if (media) {
                void media.toPreviewURL(nutstoreFile).then((previewUrl) => {
                  ...
                  ;(window['mindMapWindow'] as Window).postMessage(
                    {
                      type: 'previewUrl',
                      url: previewUrl,
                      imageData,
          ...
          case 'previewUrl':
            if (data.url && data.imageData) {
              ...
              store.dispatch(
                NodeActions.setImagePreviewUrls({
                  id: imageData.id,
                  urls: [data.url],
                }),
              )
